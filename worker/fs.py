...
from .environment import get_file_storage_system_env, Env_EndponitKey, \
    Env_AccessKey, Env_SecretKey, Env_BucketKey, get_image_prefix_key
from .redis import dist_locker, RedisLocker
from .processor import MultiThreadWorker
from multiprocessing import cpu_count
from urllib.parse import urlparse, urlsplit
from .locks import LOCK_EX, LOCK_NB, lock, unlock
from .host import get_host_name, get_host_ip
...
def download_with_oss(obj, bucket, local_path, tmp):
...
class OssFileStorage(FileStorage):
    ...
    def download_dir(self, remoting_dir: str, local_dir: str) -> bool:
        ...
        # 下载列表~
        worker = MultiThreadWorker(args, download_with_oss, 4)
        worker.run()
...
def download_file(remoting: str, local: str):
    if os.path.isfile(local):
        return True
    logging.info(f"download file from: {remoting} to:{local}")
    ...
    if remoting.lower().startswith('http'):
        ps = PrivatizationFileStorage()
        ps.download(remoting, local)
    else:
        oss = OssFileStorage()
        oss.download(remoting, local, print_progress)
    return os.path.isfile(local)


def upload_image(local: str, remoting: str = None):
    if not os.path.isfile(local):
        raise OSError(f'cannot file:{local}')
    if not remoting:
        dirname = get_image_prefix_key()
        today = datetime.datetime.now().strftime("%Y-%m-%d")
        filename = uuid.uuid4().hex
        _, ex = os.path.splitext(local)
        remoting = os.path.join(dirname, today, filename + ex)

    oss = OssFileStorage()
    return oss.upload(local, remoting)
